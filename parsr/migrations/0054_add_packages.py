...
class Migration(DataMigration):
    ...
    def update(self, package, position=0):
        package.left = position

        for child in package.children.all():
            position = self.update(child, position + 1)

        package.right = position + 1

        package.save()

        return package.right

    def forwards(self, orm):
        for f in orm.File.objects.filter():
            parent = None
            parent_name = ""
            branch = f.revision.branch

            for pkg in f.package.split("/"):
                name = "%s/%s" % (parent_name, pkg)

                try:
                    package = orm.Package.objects.get(
                        name=name,
                        parent=parent,
                        branch=branch
                    )
                except orm.Package.DoesNotExist:
                    package = orm.Package.objects.create(
                        name=name,
                        parent=parent,
                        branch=branch
                    )
        ...
        for package in orm.Package.objects.filter(parent=None):
            self.update(package)
